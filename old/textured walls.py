...
bricks_img = pygame.image.load('tiles/tile' + str(randint(1, 16)).zfill(2) + '.png').convert()

# precompute images to save time during rendering
wall_imgs = [pygame.transform.scale(bricks_img, (64, i)) for i in range(projection_plane_dimensions[1] + 1)]


def inp():
	global player_pos, player_view_angle
	...
	direction.from_polar((5, player_view_angle))
...
while True:
	...
	for column in range(projection_plane_dimensions[0]):
		...
		if closest:
			dist = math.sqrt(calulate_distance(closest, player_pos))
			beta = (player_view_angle - angle) % 360
			height = min(block_height / dist * distance_to_projection_plane / math.cos(math.radians(beta)), projection_plane_dimensions[1])
			screen.blit(wall_imgs[int(height)], (column, projection_plane_dimensions[1] // 2 - height // 2), (bitmap_offset, 0, 1, height))
			pygame.draw.line(screen, (100, 100, 100), (column, projection_plane_dimensions[1] // 2 + height // 2), (column, projection_plane_dimensions[1]))
