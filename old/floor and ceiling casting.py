...
def inp():
	global player_pos, player_view_angle, delta, projection_plane_center
	...
	if pygame.key.get_pressed()[pygame.K_DOWN]:
		projection_plane_center[1] -= 1.5 * timedelta
...
while True:
	...
	for column in range(projection_plane_dimensions[0]):
		...
		if closest:
			...
			height = min(block_height / dist * distance_to_projection_plane / math.cos(math.radians(beta)),
						 projection_plane_dimensions[1] * 10)
			screen.blit(wall_imgs[int(height)], (column, projection_plane_center[1] - height // 2),
						(bitmap_offset, 0, 1, height))

			# floor casting
			if not column % 2:
				last_bottom_of_wall = int(projection_plane_center[1] + height // 2)
				for row in range(last_bottom_of_wall, projection_plane_dimensions[1], 2):
					ratio = player_height / (row - projection_plane_center[1])
				...
				# ceiling casting
				last_top_of_wall = int(projection_plane_center[1] - height // 2)
