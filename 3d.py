...
def inp():
	...
	d = norm(rays[len(rays) // 2].direction, movementS * timedelta)
	nextPos = origin
...
rays = [Ray(i) for i in get_points(500, 800 // colW * fov)]
...
def loop():
	global origin, scene
	scene = []
	looking = rays[len(rays) // 2].direction
	for r in rays:
		r.closest = None
		record = -100
		wall = None
		for b in walls:
			if c := r.cast(b, origin):
				u = c[2]
				if record < u:
					record = u
					r.closest = c[:2]
		...
		if r.closest:
			...
			if (not r.closest[0] % 25) and r.closest[1] % 25:
				spriteIndex = r.closest[1] % 25
			elif not r.closest[1] % 25 and r.closest[0] % 25:
				spriteIndex = r.closest[0] % 25
			else:
				spriteIndex = r.closest[0] % 25
			sprite = wall.sprite
			scene.append((d * a, spriteIndex, sprite))
...
def draw():
	...
	for r in rays:
		if r.closest:
			pygame.draw.line(screen, (200, 200, 200), tuple(map(int, origin)), r.closest)
	for b in walls:
		b.show(screen)
	...
	for c in range(len(scene)):
		...
		b = scene[c][1] * colW
		img = pygame.transform.scale(scene[c][2], (25 * colW, 2 * h))
